...
function FavoritesProducts({ favorites }) {
    ...
    async function fetchData() {
      try {
        const res = await fetch("/api/product/selected-products", {
          ...
          body: JSON.stringify({ selectedIds: favorites }),
        });
        ...
        {loading ? (
          ...
          <div className="py-[30px]">
            ...
            {products.length === 0 ? (
              ...
              <div className="flex w-full flex-wrap justify-center items-center  gap-x-[10px] gap-y-[15px]">
...
export default function Page() {
  ...
  return (
    <div>
      <div>
        <FavoritesProducts favorites={JSON.parse(session.user?.favorites)} />
      </div>
    </div>
  );
}
